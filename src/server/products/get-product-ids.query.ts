...
/**
 * Get only active product IDs from the database
 * Used for displaying products on public pages
 * @returns Promise that resolves to an array of active product IDs
 */
export async function getActiveProductIds(): Promise<string[]> {
	try {
		...
		const products = await db
			.collection("products")
			.find<{ id: string }>({ status: "active" as ProductStatus }, { projection: { id: 1 } })
			.toArray();

		// Extract IDs from MongoDB documents
		return products.map((doc) => doc.id);
	} catch (error) {
		console.error("Error fetching active product IDs:", error);
		throw new Error("Failed to fetch active product IDs");
	}
}
