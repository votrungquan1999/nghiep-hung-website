...
interface ProjectImageRendererProps extends React.ComponentProps<typeof Image> {
	file: {
		id: string;
		preview: string;
		name: string;
...
/**
 * Custom image renderer for project creation with main image selection
 ...
 */
export function ProjectImageRenderer({ file, ...imageProps }: ProjectImageRendererProps) {
	const { selectedMainImageId, setMainImage } = useCreateProjectMainImage();

	const isMainImage = selectedMainImageId === file.id;
